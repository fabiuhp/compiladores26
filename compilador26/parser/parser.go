...
var ruleDescriptions = map[int]string{
	1: "Program -> FunctionList", 2: "FunctionList -> Function FunctionList'",
	3: "FunctionList' -> Function FunctionList'", 4: "FunctionList' -> e",
	5: "Function -> Type id ( ParamListOpt ) Block",
	6: "ParamListOpt -> ParamList", 7: "ParamListOpt -> e",
	8: "ParamList -> Param ParamList'", 9: "ParamList' -> , Param ParamList'",
	10: "ParamList' -> e", 11: "Param -> Type id",
	12: "Block -> { DeclListOpt StmtListOpt }",
	13: "DeclListOpt -> DeclList", 14: "DeclListOpt -> e",
	15: "DeclList -> VarDecl DeclList'", 16: "DeclList' -> VarDecl DeclList'",
	17: "DeclList' -> e", 18: "VarDecl -> Type id ;",
	19: "StmtListOpt -> StmtList", 20: "StmtListOpt -> e",
	21: "StmtList -> Stmt StmtList'", 22: "StmtList' -> Stmt StmtList'",
	23: "StmtList' -> e", 24: "Stmt -> AssignStmt",
	25: "Stmt -> IfStmt", 26: "Stmt -> WhileStmt",
	27: "Stmt -> PrintStmt", 28: "Stmt -> ReturnStmt",
	29: "Stmt -> Block", 30: "AssignStmt -> id = Expr ;",
	31: "ReturnStmt -> return Expr ;", 32: "PrintStmt -> print ( Expr ) ;",
	33: "IfStmt -> if ( Expr ) Stmt ElsePart",
	34: "ElsePart -> else Stmt", 35: "ElsePart -> e",
	36: "WhileStmt -> while ( Expr ) Stmt",
	37: "Expr -> RelExpr", 38: "RelExpr -> AddExpr RelExpr'",
	39: "RelExpr' -> RelOp AddExpr", 40: "RelExpr' -> e",
	41: "RelOp -> ==", 42: "RelOp -> !=", 43: "RelOp -> <",
	44: "RelOp -> >", 45: "RelOp -> <=", 46: "RelOp -> >=",
	47: "AddExpr -> MulExpr AddExpr'",
	48: "AddExpr' -> + MulExpr AddExpr'", 49: "AddExpr' -> - MulExpr AddExpr'",
	50: "AddExpr' -> e", 51: "MulExpr -> Factor MulExpr'",
	52: "MulExpr' -> * Factor MulExpr'", 53: "MulExpr' -> / Factor MulExpr'",
	54: "MulExpr' -> e", 55: "Factor -> ( Expr )",
	56: "Factor -> id FactorTail", 57: "Factor -> num",
	58: "FactorTail -> ( ArgListOpt )", 59: "FactorTail -> e",
	60: "ArgListOpt -> ArgList", 61: "ArgListOpt -> e",
	62: "ArgList -> Expr ArgList'", 63: "ArgList' -> , Expr ArgList'",
	64: "ArgList' -> e", 65: "Type -> int", 66: "Type -> float",
}

var parseTable = map[int]map[int]int{
	token.NTProgram:      {token.INT: 1, token.FLOAT: 1},
	token.NTFunctionList: {token.INT: 2, token.FLOAT: 2},
	token.NTFunctionListP: {
		token.INT: 3, token.FLOAT: 3, token.EOF: 4,
	},
	token.NTFunction:     {token.INT: 5, token.FLOAT: 5},
	token.NTParamListOpt: {token.INT: 6, token.FLOAT: 6, token.RPAREN: 7},
	token.NTParamList:    {token.INT: 8, token.FLOAT: 8},
	token.NTParamListP:   {token.COMMA: 9, token.RPAREN: 10},
	token.NTParam:        {token.INT: 11, token.FLOAT: 11},
	token.NTBlock:        {token.LBRACE: 12},
	token.NTDeclListOpt: {
	...
	token.NTDeclList:  {token.INT: 15, token.FLOAT: 15},
	token.NTDeclListP: {
	...
	token.NTVarDecl: {token.INT: 18, token.FLOAT: 18},
	token.NTStmtListOpt: {
	...
	token.NTWhileStmt: {token.WHILE: 36},
	token.NTExpr:      {token.LPAREN: 37, token.ID: 37, token.NUM: 37},
	token.NTRelExpr:   {token.LPAREN: 38, token.ID: 38, token.NUM: 38},
	token.NTRelExprP: {
	...
	token.NTAddExpr: {token.LPAREN: 47, token.ID: 47, token.NUM: 47},
	token.NTAddExprP: {
	...
	token.NTMulExpr: {token.LPAREN: 51, token.ID: 51, token.NUM: 51},
	token.NTMulExprP: {
	...
	token.NTFactor: {token.LPAREN: 55, token.ID: 56, token.NUM: 57},
	token.NTFactorTail: {
		token.LPAREN: 58,
		token.MULT: 59, token.DIV: 59, token.PLUS: 59, token.MINUS: 59,
		token.EQ: 59, token.NEQ: 59, token.LT: 59, token.GT: 59,
		token.LTE: 59, token.GTE: 59,
		token.SEMI: 59, token.RPAREN: 59, token.COMMA: 59,
	},
	token.NTArgListOpt: {
		token.LPAREN: 60, token.ID: 60, token.NUM: 60, token.RPAREN: 61,
	},
	token.NTArgList:  {token.LPAREN: 62, token.ID: 62, token.NUM: 62},
	token.NTArgListP: {token.COMMA: 63, token.RPAREN: 64},
	token.NTType:     {token.INT: 65, token.FLOAT: 66},
}
...
func New(tokens []token.Token) *Parser {
	...
	copy(toks, tokens)
	toks = append(toks, token.Token{Code: token.EOF, Lexeme: "$"})
...
func (p *Parser) currentToken() token.Token {
	...
	return token.Token{Code: token.EOF, Lexeme: "$"}
}
		...
		if top == token.EOF {
			if cur.Code == token.EOF {
				...
				fmt.Printf("%-6d %-20s %-6s %-50s %s\n",
					p.step, p.fmtToken(cur), "", "ACEITA", p.fmtStack())
				return nil
			}
		...
		if token.IsTerminal(top) {
			if top == cur.Code {
				...
				fmt.Printf("%-6d %-20s %-6d %-50s %s\n",
					p.step, p.fmtToken(cur), cur.Line,
					fmt.Sprintf("Consome: %s", cur.Lexeme), p.fmtStack())
				p.pos++
			} else {
			...
			if !exists {
				return fmt.Errorf("linha %d: nao-terminal %s sem entradas na tabela",
...
func (p *Parser) syntaxError(nt int, cur token.Token) error {
	var expected []string
	if row, ok := parseTable[nt]; ok {
		for code := range row {
			expected = append(expected, "'"+token.TerminalName(code)+"'")
		}
	}
	if len(expected) > 0 {
		return fmt.Errorf("linha %d: erro sintatico em %s - esperado %s, encontrou '%s'",
			cur.Line, token.NonTerminalName(nt), strings.Join(expected, " ou "), cur.Lexeme)
	}
	return fmt.Errorf("linha %d: erro sintatico - token inesperado '%s'", cur.Line, cur.Lexeme)
}

func (p *Parser) fmtToken(t token.Token) string {
	if t.Code == token.EOF {
		return "$ (27)"
	}
